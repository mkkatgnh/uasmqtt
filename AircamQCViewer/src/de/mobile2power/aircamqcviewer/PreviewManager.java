...
public class PreviewManager {
	...
	public void draw(Canvas canvas, Preview preview) {
		...
		if (jpegData != null && preview.getWidth() > 0
				&& preview.getHeight() > 0) {
			Bitmap previewFrame = BitmapFactory.decodeByteArray(jpegData, 0,
					jpegData.length);
			if (previewFrame != null) {
				Bitmap scaledBitmap = Bitmap.createScaledBitmap(previewFrame,
						width, height, false);
				canvas.drawBitmap(scaledBitmap, upperLeftX, upperLeftY, null);
			}
		}
	}
}
